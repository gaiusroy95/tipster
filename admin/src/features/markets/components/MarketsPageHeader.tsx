...
export function MarketsPageHeader({
  ...
  const stats = [
    {
      label: 'Arena controls',
      value: arenaTotal,
      caption: 'Categories shown on the public site',
      icon: Squares2X2Icon,
    ...
    },
    {
      label: 'Live in arena',
      value: enabled,
      caption: 'Enabled for players right now',
      icon: EyeIcon,
      accent: 'win' as const,
    },
    {
      label: 'Overtime catalog',
      value: overtimeTotal,
      caption: `${extendedCount} extended · mapped to arena categories`,
      icon: EyeSlashIcon,
  ...
  return (
    <section className="dashboard-hero relative overflow-hidden rounded-3xl border border-border-default/60 p-5 sm:p-7">
      ...
      <div
        className="pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full bg-accent-primary/10 blur-3xl"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute -bottom-16 -left-8 h-40 w-40 rounded-full bg-accent-win/10 blur-3xl"
        aria-hidden="true"
      />
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
        <div className="space-y-5">
          ...
          <div>
            <h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">Markets</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-text-muted sm:text-base">
              Control which odd categories appear in Tipster Arena. Overtime supplies many market
              types — they roll up into 1X2, handicap, and over/under on fixture cards. Enable the
              categories you want players to see and bet on.
            </p>
          </div>
        ...
        <Button size="lg" className="w-full lg:w-auto" isLoading={isSyncing} onClick={onSync}>
          <ArrowPathIcon className="h-4 w-4" aria-hidden="true" />
          Sync Overtime catalog
        </Button>
      </div>
    </section>
  )
